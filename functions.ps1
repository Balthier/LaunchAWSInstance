...
function verifyImageId($ImageName) {
	...
	$ImageId = $image | Select -ExpandProperty ImageId
	$Name = $image | Select -ExpandProperty Name
	$Description = $image | Select -ExpandProperty Description
	$State = $image | Select -ExpandProperty State
	$CreatedDate = $image | Select -ExpandProperty CreationDate
...
function AWSAuth($profile,$region) {
